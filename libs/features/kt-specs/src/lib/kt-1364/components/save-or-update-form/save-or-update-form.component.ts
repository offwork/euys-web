...
import { Celik, KtAtDurulama, KtSpDurulama, Urun } from '@euys/api-interfaces';
import {
...
export class SaveOrUpdateFormComponent implements OnInit, OnDestroy {
  ...
  @Input() anatabloKomboList$: Observable<KtAtDurulama[]>;
  ...
  @Input() selectedRow: KtSpDurulama;
  @Input() isFormVisible = true;
  @Input() update!: boolean;
  @Output() formSubmit = new EventEmitter<KtSpDurulama>();
  ...
  urunler: Urun[];
  celikler: Celik[];
  anatablo: KtAtDurulama[];
  formData: KtSpDurulama;
  urunTarget: Urun[];
  celikTarget: Celik[];
  pickListDirty = false;
  ...
  ngOnInit() {
    ...
    this.anatabloKomboList$
      .pipe(takeUntil(this._endSubcription$))
      .subscribe((anatabloKomboList: KtAtDurulama[]) => {
  ...
  setAnatabloKomboList(anatabloKomboList?: KtAtDurulama[]): void {
  ...
  onSubmit() {
    ...
    if (
      ...
      if (this.saveOrUpdateForm.getRawValue().durulamaKodu === null) {
        this.toastMessageService.warning(
          "Durulama Spesifikasyonu Ana Tablo'dan seçim yapılmalı"
        );
      }
    }
  }
  ...
  get durulamaKodu() {
    return this.saveOrUpdateForm.get('durulamaKodu');
  }
}
