...
export class SaveOrUpdateFormComponent implements OnInit, OnDestroy {
  ...
  @Input() anatabloKomboList$: Observable<KtAtBobinDilUcu[]>;
  ...
  @Input() selectedRow: KtSpBobinDilUcu;
  @Input() isFormVisible = true;
  @Input() update!: boolean;
  @Output() formSubmit = new EventEmitter<KtSpBobinDilUcu>();
  ...
  urunler: Urun[];
  celikler: Celik[];
  anatablo: KtAtBobinDilUcu[];
  formData: KtSpBobinDilUcu;
  urunTarget: Urun[];
  celikTarget: Celik[];
  pickListDirty = false;
  ...
  ngOnInit() {
    ...
    this.anatabloKomboList$
      .pipe(takeUntil(this._endSubcription$))
      .subscribe((anatabloKomboList: KtAtBobinDilUcu[]) => {
  ...
  setAnatabloKomboList(anatabloKomboList?: KtAtBobinDilUcu[]): void {
  ...
  onSubmit() {
    ...
    if (
      ...
      if (this.saveOrUpdateForm.getRawValue().bobinDilUcuKodu === null) {
        this.toastMessageService.warning(
          "Bobin Dil Ucu Spesifikasyonu Ana Tablo'dan seçim yapılmalı"
        );
      }
    }
  }
  ...
  get bobinDilUcuKodu() {
    return this.saveOrUpdateForm.get('bobinDilUcuKodu');
  }
}
