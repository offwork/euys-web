...
export class SaveOrUpdateFormComponent implements OnInit, OnDestroy {
  ...
  @Input() anatabloKomboList$: Observable<KtAtAlkaliTemizleme[]>;
  ...
  @Input() selectedRow: KtSpAlkaliTemizleme;
  @Input() isFormVisible = true;
  @Input() update!: boolean;
  @Output() formSubmit = new EventEmitter<KtSpAlkaliTemizleme>();
  ...
  urunler: Urun[];
  celikler: Celik[];
  anatablo: KtAtAlkaliTemizleme[];
  formData: KtSpAlkaliTemizleme;
  urunTarget: Urun[];
  celikTarget: Celik[];
  pickListDirty = false;
  ...
  ngOnInit() {
    ...
    this.anatabloKomboList$
      .pipe(takeUntil(this._endSubcription$))
      .subscribe((anatabloKomboList: KtAtAlkaliTemizleme[]) => {
  ...
  setAnatabloKomboList(anatabloKomboList?: KtAtAlkaliTemizleme[]): void {
  ...
  onSubmit() {
    ...
    if (
      ...
      if (this.saveOrUpdateForm.getRawValue().alkaliTemizlemeKodu === null) {
        this.toastMessageService.warning(
          "ALKALI TEMİZLEME Spesifikasyonu Ana Tablo'dan seçim yapılmalı"
        );
      }
    }
  }

  get alkaliTemizlemeKodu() {
    return this.saveOrUpdateForm.get('alkaliTemizlemeKodu');
  }
}
