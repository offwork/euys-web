...
export class SaveOrUpdateFormComponent implements OnInit, OnDestroy {
  ...
  @Input() anatabloKomboList$: Observable<
    KtAtEnineKalinlikVeIkiKenarFarklari[]
  >;
  ...
  @Input() selectedRow: KtSpEnineKalinlikVeIkiKenarFarklariSpec;
  ...
  urunler: Urun[];
  celikler: Celik[];
  anatablo: KtAtEnineKalinlikVeIkiKenarFarklari[];
  formData: KtSpEnineKalinlikVeIkiKenarFarklariSpec;
  urunTarget: Urun[];
  celikTarget: Celik[];
  pickListDirty = false;
  ...
  constructor(
    ...
    this.saveOrUpdateForm = this.formBuilder.group({
      ...
      maxGenislik: null,
      minGenislik: null,
      enineKalinIkiKenarFarkKodu: null,
  ...
  ngOnInit() {
    ...
    this.anatabloKomboList$
      .pipe(takeUntil(this._destroy$))
      .subscribe((anatabloKomboList: KtAtEnineKalinlikVeIkiKenarFarklari[]) => {
  ...
  save() {
    ...
    if (
      ...
      if (
        this.saveOrUpdateForm.getRawValue().enineKalinIkiKenarFarkKodu === null
      ) {
        this.toastMessageService.warning(
          "Enine Kalınlık ve İki Kenar Ana Tablo'dan seçim yapılmalı"
        );
      }
    }
  }

  get enineKalIkiKenarFarkKodu() {
    return this.saveOrUpdateForm.get('enineKalinIkiKenarFarkKodu');
  }
