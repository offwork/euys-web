...
export class SaveOrUpdateFormComponent implements OnInit, OnDestroy {
  ...
  @Input() anatabloKomboList$: Observable<KtAtIiTenekeKalayKaplama[]>;
  ...
  @Input() selectedRow: KtSpIiTenekeKalayKaplama;
  @Input() isFormVisible = true;
  @Input() update!: boolean;
  @Output() formSubmit = new EventEmitter<KtSpIiTenekeKalayKaplama>();
  ...
  urunler: Urun[];
  celikler: Celik[];
  anatablo: KtAtIiTenekeKalayKaplama[];
  formData: KtSpIiTenekeKalayKaplama;
  urunTarget: Urun[];
  celikTarget: Celik[];
  pickListDirty = false;
  ...
  ngOnInit() {
    ...
    this.anatabloKomboList$
      .pipe(takeUntil(this._destroy$))
      .subscribe((anatabloKomboList: KtAtIiTenekeKalayKaplama[]) => {
  ...
  setAnatabloKomboList(anatabloKomboList?: KtAtIiTenekeKalayKaplama[]) {
  ...
  save() {
    ...
    if (
      ...
      if (
        this.saveOrUpdateForm.getRawValue().iiTenekeKalayKaplamaKodu === null
      ) {
        this.toastMessageService.warning(
          "II. Teneke Hattı Kalay Kaplama Spesifikasyonu Ana Tablo'dan seçim yapılmalı"
        );
      }
    }
  }

  get iiTenekeKalayKaplamaKodu() {
    return this.saveOrUpdateForm.get('iiTenekeKalayKaplamaKodu');
  }
