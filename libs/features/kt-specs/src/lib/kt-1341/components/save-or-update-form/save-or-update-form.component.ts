...
export class SaveOrUpdateFormComponent implements OnInit, OnDestroy {
  ...
  @Input() anatabloKomboList$: Observable<KtAtKromKaplamaTfsFlor[]>;
  ...
  @Input() selectedRow: KtSpKromKaplamaTfsFlor;
  @Input() isFormVisible = true;
  @Input() update!: boolean;
  @Output() formSubmit = new EventEmitter<KtSpKromKaplamaTfsFlor>();
  ...
  urunler: Urun[];
  celikler: Celik[];
  anatablo: KtAtKromKaplamaTfsFlor[];
  formData: KtSpKromKaplamaTfsFlor;
  urunTarget: Urun[];
  celikTarget: Celik[];
  pickListDirty = false;
  ...
  setAnatabloKomboList(anatabloKomboList?: KtAtKromKaplamaTfsFlor[]): void {
  ...
  onSubmit() {
    ...
    if (
      ...
      if (
        this.saveOrUpdateForm.getRawValue().kromKaplamaTfsFlorKodu === null
      ) {
  ...
  get kromKaplamaTfsFlorKodu() {
    return this.saveOrUpdateForm.get('kromKaplamaTfsFlorKodu');
  }
}
