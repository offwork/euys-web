...
export class SaveOrUpdateFormComponent implements OnInit, OnDestroy {
  ...
  @Input() anatabloKomboList$: Observable<KtAtKromKaplamaTfsDragout[]>;
  ...
  @Input() selectedRow: KtSpKromKaplamaTfsDragout;
  @Input() isFormVisible = true;
  @Input() update!: boolean;
  @Output() formSubmit = new EventEmitter<KtSpKromKaplamaTfsDragout>();
  ...
  urunler: Urun[];
  celikler: Celik[];
  anatablo: KtAtKromKaplamaTfsDragout[];
  formData: KtSpKromKaplamaTfsDragout;
  urunTarget: Urun[];
  celikTarget: Celik[];
  pickListDirty = false;
  ...
  setAnatabloKomboList(anatabloKomboList?: KtAtKromKaplamaTfsDragout[]): void {
  ...
  onSubmit() {
    ...
    if (
      ...
      if (
        this.saveOrUpdateForm.getRawValue().kromKaplamaTfsDragOutKodu === null
      ) {
  ...
  get kromKaplamaTfsDragOutKodu() {
    return this.saveOrUpdateForm.get('kromKaplamaTfsDragOutKodu');
  }
}
