...
export class SaveOrUpdateFormComponent implements OnInit, OnDestroy {
  ...
  @Input() anatabloKomboList$: Observable<KtAtAsitlemeTank[]>;
  ...
  @Input() selectedRow: KtSpAsitlemeTank;
  @Input() isFormVisible = true;
  @Input() update!: boolean;
  @Output() formSubmit = new EventEmitter<KtSpAsitlemeTank>();
  ...
  urunler: Urun[];
  celikler: Celik[];
  anatablo: KtAtAsitlemeTank[];
  formData: KtSpAsitlemeTank;
  urunTarget: Urun[];
  celikTarget: Celik[];
  pickListDirty = false;
  ...
  ngOnInit() {
    ...
    this.anatabloKomboList$
      .pipe(takeUntil(this._endSubcription$))
      .subscribe((anatabloKomboList: KtAtAsitlemeTank[]) => {
  ...
  setAnatabloKomboList(anatabloKomboList?: KtAtAsitlemeTank[]): void {
  ...
  onSubmit() {
    ...
    if (
      ...
      if (this.saveOrUpdateForm.getRawValue().asitlemeTankKodu === null) {
        this.toastMessageService.warning(
          "Asitleme Tank  Spesifikasyonu Ana Tablo'dan seçim yapılmalı"
        );
      }
    }
  }

  get asitlemeTankKodu() {
    return this.saveOrUpdateForm.get('asitlemeTankKodu');
  }
}
