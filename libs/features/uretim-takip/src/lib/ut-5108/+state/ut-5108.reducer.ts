...
export const UT_5108_FEATURE_KEY = 'ut5108';
...
const ut5108Reducer = createReducer(
  initialState,
  on(Ut5108Actions.init, state => ({
    ...state,
    loaded: false,
    error: null,
    record: null,
  })),
  on(Ut5108Actions.loadUt5108Success, (state, { data }) => ({
    ...state,
    loaded: true,
  ...
  on(Ut5108Actions.loadUt5108LogSuccess, (state, { data }) => ({
    ...state,
    dataLog: data,
  })),
  on(
  ...
  on(Ut5108Actions.saveUt5108Failure, (state, { error }) => ({
    ...state,
    error,
    loaded: true,
  }))
...
export function reducer(state: State | undefined, action: Action) {
  return ut5108Reducer(state, action);
}
